...
namespace Lib_System.Services
{
    public class LogUserService : ILogUserService
    {
        ...
        public IEnumerable<LogUserViewModel> GetAllUserDetails()
        {
            using IDbConnection c = _db.GetConnection();
            c.Open();
            return c.Query<LogUserViewModel>(@"
                SELECT 
                    id           AS Id,
        ...
        public LogUser GetUserById(int id)
        {
            using IDbConnection c = _db.GetConnection();
            c.Open();
            return c.QuerySingle<LogUser>(@"
                SELECT 
                    id             AS Id,
                    username       AS Username,
                    email          AS Email,
        ...
        public bool DeleteUser(int id)
        {
            ...
            c.Open();
            return c.Execute(
                "DELETE FROM MA_log_users WHERE id = @Id",
                new { Id = id }) > 0;
        }
    }
}